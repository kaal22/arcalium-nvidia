...
- **Updates and Recovery:** `updates check|apply|rollback|reboot` open `/usr/lib/arcalium/updates/session.sh` in a terminal so `sudo bootc …` can prompt for a password and show progress (Control Centre never runs privileged bootc silently). Apply/rollback require typing `yes` in the terminal before continuing. Updates page shows `bootc` status and copyable apply/rollback commands but does not run them. Diagnostics can write a redacted bundle under `~/.local/state/arcalium/`. Local AI launches a terminal session (does not keep the model warm after close). Quick actions launch allowlisted `.desktop` files via `gio launch` (fallback `gtk-launch` / `kioclient exec`) — never `xdg-open` on the path.
