...
Konsole’s banner is the interactive MOTD (`/etc/profile.d/user-motd.sh`). We replaced Bazzite’s tip markdown with `fastfetch` using `/usr/share/arcalium/fastfetch.jsonc` and the ASCII mark in `/usr/share/arcalium/logo.txt`. `fastfetch` / `neofetch` aliases (bash `bazzite-neofetch.sh` and fish `vendor_conf.d`) point at the same config. Build also overwrites `/usr/share/ublue-os/bazzite/fastfetch.jsonc` + `logo.txt`, and rewires `ublue-motd` / `bazzite-fetch-image`, so upstream hooks cannot silently show Bazzite after a re-pin. The image line is `/usr/libexec/arcalium-image-label`, which prefers the **booted** `bootc` / `rpm-ostree` image tag (`:stable`, `:dev`, `:0.x.y`) over baked `/etc/arcalium/image-info.json` — promote retags the same digest, so the JSON channel stays `dev` unless a rebuild sets `ARCALIUM_CHANNEL`. Per-user opt-out is unchanged: `touch ~/.config/no-show-user-motd` (or `ujust toggle-user-motd`).
