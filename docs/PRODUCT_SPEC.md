...
**Arcalium decision (Phase 9):** the published image **does not ship Steam**. `build.sh` removes the inherited Bazzite `steam` / `steam-devices` RPMs and asserts `steam.desktop` is gone. Users obtain Steam on demand:
