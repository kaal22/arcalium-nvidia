...
| 2026-08-23 | **Re-pin branding blow-up on 3060** | After base `0fba65cb…`, splash/login, Kickoff mark, Bazzite Portal/apps, truncated `bazzite-neofetch.sh` / `ublue-motd`, blank Konsole (`case $- in i)` + non-login profiles), and slimmed fastfetch all bit. Documented as **Lesson: 2026-08-23** in `docs/REPIN_BASE.md`; durable fixes: force-rewrite MOTD hooks, Konsole `arcalium-konsole-shell`, `[[ $- =~ i ]]`, full fastfetch, logo theme overrides, harder launcher scrub. Follow-up: brand **all** `Splash.qml` (not only vapor), default `LookAndFeelPackage=vapor`, mirror Kickoff PNGs into Breeze, scrub “Update your system”. |
